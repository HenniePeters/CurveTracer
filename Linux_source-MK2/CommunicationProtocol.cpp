...
// Prior to this version it was calculated every time a byte was added to the structure, keeping the checksum correct at all times.
// Because the checksum is sent as the last byte of the package, it can easily be calculated during transmission. The checksum of
// the package is now correct at the end of the functions "pkg_send" and "pkg_receive".
//
//######################################################################################################
//
...
// 55 33                    <-  the header containing bytes: 0x55 = Bin:01010101 Dec:85 'U' and 0x33 = Bin:00110011 Dec:51 '3'
//       00 01              <-  the number of data bytes, MSB first, LSB last (macros MSB and LSB are defined in CommunicationProtocol.h)
...
// Mind that (fifo)buffer size needs space for communication_package.size + 5 ALSO data bytes equal to headerbyte1 need to be doubled!
//
//######################################################################################################
extern uint8_t send_byte( uint8_t c );
extern uint8_t read_byte( void );
