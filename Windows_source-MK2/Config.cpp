...
void AppConfigData::ReadOptions( char *sIniFile ) {
    ...
    try {
        ...
        GetPrivateProfileString( "options", "colour3", "16777215", pszBuffer, iFileLength, sIniFile );
        Options["colour3"] = pszBuffer;
...
void AppConfigData::Save( void ) {
    if( ! ReadOnly() ) {
        ...
        if( Options["colour3"] != "" ) {
            as = Options["colour3"].c_str();
            WritePrivateProfileString( "options", "colour3", as.c_str(), wxsConfigPath.c_str() );
        }
        if( Options["interval"] != "" ) {
