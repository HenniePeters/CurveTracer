...
PaintGraphics::PaintGraphics( TImage *stArea_param ) {
    ...
    wxPen_Ruler->Color = (TColor)RGB(128,128,128);
    wxBrush_Ruler->Color = (TColor)RGB(0,0,0);
    wxPen_Grid20->Color = (TColor)RGB(32,32,32);
    wxPen_Grid40->Color = (TColor)RGB(64,64,64);
    wxPen_Grid80->Color = (TColor)RGB(128,128,128);
    wxPen_Border->Color = (TColor)RGB(255,255,255);
    wxBrush_Border->Color = (TColor)RGB(0,0,0);
    wxPen_Margin->Color = (TColor)RGB(0,0,0);
    wxBrush_Margin->Color = (TColor)RGB(0,0,0);
    Brush_Samples->Color = (TColor)RGB(0,0,0);
    wxPen_SamplesGreen->Color = (TColor)RGB(0,0xCC,0); //(TColor)0x0000CC00;
    wxPen_SamplesBlue->Color = (TColor)RGB(255,64,64); //(TColor)0x00FF4040;

    // text settings
    canvas->Font->Color = (TColor)RGB(128,128,128);
    canvas->Font->Size = 10;
...
void PaintGraphics::ShowSamples( struct f *fields, struct f *held, bool bShowHeld ) {
    ...
    a = 2+map( fields->xy[0], 0-fields->Vx, fields->Vx, iLeftEdge+1, iRightEdge-1 );
    e = a;
    b = 2+map( fields->xy[1], 0-fields->Vx, fields->Vy, iTopEdge+1, iBottomEdge-1 );
    f = b;
    for( int i = 2; i < (COORDS<<1); i+=2 ) {
        c = map( fields->xy[i], 0-fields->Vx, fields->Vx, iLeftEdge, iRightEdge );
        d = map( fields->xy[i+1], 0-fields->Vy, fields->Vy, iTopEdge, iBottomEdge );
        DrawLine( canvas, a, b, c, d );
        a = c;
        b = d;
    }
...
//---------------------------------------------------------------------------
void PaintGraphics::ChangeSamplesColour( unsigned long clEven, unsigned long clOdd, unsigned long clHeld ) {
    wxPen_SamplesGreen->Color = (TColor)clEven;
    wxPen_SamplesBlue->Color = (TColor)clOdd;
    wxPen_SamplesHeld->Color = (TColor)clHeld;
}
//---------------------------------------------------------------------------
void __fastcall PaintGraphics::SetPenWidth(int value) {
    ...
    wxPen_SamplesGreen->Width = iPenWidth;
    wxPen_SamplesBlue->Width = iPenWidth;
    wxPen_SamplesHeld->Width = iPenWidth;
}
//---------------------------------------------------------------------------
